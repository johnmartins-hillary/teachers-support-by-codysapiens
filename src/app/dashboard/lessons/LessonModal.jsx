  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center z-50 text-gray-500" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-md p-6 w-1/2">
        ...
        {lesson.lessonSection === 'text' && (
          <p className="text-gray-600">{lesson.description}</p>
        ) }{ lesson.lessonSection === 'video' && (
          <div>
            <h3 className="text-lg font-bold mb-2">Video:</h3>
            <iframe controls title={lesson.title} src={lesson?.media} width="100%" height="315" frameborder="0" allowfullscreen>Your browser does not support the video element.</iframe>
          </div>
        )} {lesson.lessonSection === 'audio' && (
          <div>
